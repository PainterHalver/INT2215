...
void Cell::show(SDL_Renderer* renderer, SDL_Texture** numberTexture) {
    ...
    if(this->revealed){
        if(this->hasBomb){
            SDL_RenderCopy(renderer, numberTexture[8], NULL, &smallRect);
...
void Cell::hover(SDL_Renderer* renderer){
    if(this->revealed || this->isHovered || this->isFlagged) return;
    this->isHovered = true;
...
void Cell::reveal(Cell** grid, int ROWS, int COLS, SDL_Renderer* renderer, SDL_Texture* bombTexture, SDL_Texture** numberTexture){
    if(this->revealed || this->isFlagged) return;
    this->revealed = true;
    this->show(renderer, numberTexture);
...
void Cell::toggleFlag(SDL_Renderer* renderer, SDL_Texture** numberTexture) {
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);   // black
    SDL_Rect rect = {x,y,w,h};
    SDL_RenderDrawRect(renderer, &rect);
    SDL_Rect smallRect = {x+1.3,y+1.3,w-2,h-2};
    this->isFlagged = !this->isFlagged;
    if(this->isFlagged && !this->revealed) {
        this->isHovered = false;
        SDL_RenderCopy(renderer, numberTexture[10], NULL, &smallRect);
    } else {
        this->hover(renderer);
